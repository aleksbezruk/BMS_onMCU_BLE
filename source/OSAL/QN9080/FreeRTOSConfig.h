...
#define configTIMER_TASK_PRIORITY               (configMAX_PRIORITIES - OSAL_TIMER_TASK_PRIORITY)
...
/*! 
 * @brief Interrupt priorities used by the kernel port layer itself.  These are generic
 * to all Cortex-M ports, and do not rely on any particular library functions. 
 */
#define configKERNEL_INTERRUPT_PRIORITY (configLIBRARY_LOWEST_INTERRUPT_PRIORITY << (8 - configPRIO_BITS))

/*! 
 * @attention !!!! configMAX_SYSCALL_INTERRUPT_PRIORITY must not be set to zero !!!
 * @ref See http://www.FreeRTOS.org/RTOS-Cortex-M3-M4.html.
 * 
 * @brief The maximum interrupt priority from which FreeRTOS API functions
 * (that end in "FromISR") can be called.  Interrupts that use the FreeRTOS API must not
 * be left at their default priority of zero as that is the highest possible
 * priority, which is guaranteed to be above configMAX_SYSCALL_INTERRUPT_PRIORITY,
