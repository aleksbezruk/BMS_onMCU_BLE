...
typedef struct {
    ...
    uint16_t aiosFullBatteryValueHandle;    /*< AIOS Full Battery characteristic value handle */
    uint16_t aiosBank1ValueHandle;          /*< AIOS Bank 1 characteristic value handle */
...
/*! Define minimal NVM data storage */
typedef struct {
...
/*! 
 * Enable extended logging for BLE events.
...
static bleResult_t _HAL_BLE_HostToControllerInterface(hciPacketType_t packetType, void* pPacket, uint16_t packetSize);
static void _HAL_BLE_SetupAdvertisingParameters(void);
static void _HAL_BLE_SetupAdvertisingData(void);
static void _HAL_BLE_StartAdvertisingInternal(void);
static void _HAL_BLE_AdvertisingCallback(gapAdvertisingEvent_t* pAdvertisingEvent);
static void _HAL_BLE_ConnectionCallback(deviceId_t peerDeviceId, gapConnectionEvent_t* pConnectionEvent);
static void _HAL_BLE_GattServerCallback(deviceId_t deviceId, gattServerEvent_t* pServerEvent);
static void _HAL_BLE_InitializeGattDatabase(void);
static void _HAL_BLE_DiscoverGattHandles(void);
static void _HAL_BLE_GenericCallback(gapGenericEvent_t* pGenericEvent);
...
/*! Advertising data: default Flags */
static uint8_t adData0[1] = { (gapAdTypeFlags_t)(gLeGeneralDiscoverableMode_c | gBrEdrNotSupported_c) };
/*! Advertising data: default Local Name */
static char adData1[HAL_BLE_LOCAL_NAME_MAX_LEN] = "QN9080_BMS";
/*! Advertising data: default
 * complete list of 16-bit UUIDs: Battery Service (0x180F) and Automation IO Service (0x1815)
 */
static uint8_t adData2[4] = { 0x0F, 0x18, 0x15, 0x18 }; // Little-endian format
/*! Advertising data: default
...
static uint8_t scanRspData0[1] = { 0x00 }; // 0 dBm TX power
/*! Scan Response Data structures */
static gapAdStructure_t scanRspStruct[] = {
    {
...
/*! Global variable to store advertising update state */
static volatile adv_update_state_t advUpdateState = ADV_UPDATE_IDLE;
/*! Global variable to store pending battery level for advertising */
static volatile uint8_t pendingBatteryLevel = 0U;

/*! Track advertising state for proper state management */
...
void HAL_BLE_init(void)
{
    ...
    /** Init NXP timer manager */
    TMR_Init();

    /**
     * @note In general SecLib_Init() is used for BLE security features. 
     *       But looks like there is dependency of BLE stack on the security lib found 
     *       during BLE stack bring-up. So it is recommended to call SecLib_Init() even if
     *       you don't plan to use security features.
    ...
    /** Initialize QN9080 BLE Controller. 
     * Requires that MEM_Manager and SecLib to be already initialized 
     */
    ...
    bleResult_t hostResult = Ble_HostInitialize(_HAL_BLE_GenericCallback, _HAL_BLE_HostToControllerInterface);
 ...
 * @note #1: Default advertising and scan response parameters are defined in 
 *       \ref advScanStruct and \ref g_scanRspData.
 *       These defaults are set during the BLE stack initialization.
 * @note #2: Advertise will be started after adv data is set (see event gAdvertisingDataSetupComplete_c).
 * @note #3: The App should stop advertising before starting a new advertisement by calling
 *           \ref HAL_BLE_stopAdvertisement.
...
HAL_BLE_status_t HAL_BLE_startAdvertisement(HAL_BLE_adv_data_t *advData, HAL_BLE_scan_response_data_t *scanResponseData)
{
    ...
    if (advData != NULL) {
        /** Set Flags */
        adData0[0] = (uint8_t) advData->flags;
        ...
        advScanStruct[1].length = name_len + 1U;
        memcpy((uint8_t *)adData1, (uint8_t *)advData->local_name, name_len);
        /** Update services list */
        HAL_ASSERT((advData->services > HAL_BLE_AD_SERVICE_NONE), __FILE__, __LINE__);
        if (advData->services == HAL_BLE_AD_SERVICE_BAS) {
           adData2[0] = 0x0F;
           adData2[1] = 0x18;
           advScanStruct[2].length = 3U;    // 2+1
            ...
            // Both included
            advScanStruct[2].length = 5U;    // 4+1
            adData2[0] = 0x0F;
            adData2[1] = 0x18;
            adData2[2] = 0x15;
            adData2[3] = 0x18;
        }
        /** Update service data (Battery Level in percent) */
        adData3[2] = advData->service_data;
    }

    /** Set Scan Response Data */
    if (scanResponseData != NULL) {
        scanRspData0[0] = *scanResponseData;
    }
...
HAL_BLE_status_t HAL_BLE_stopAdvertisement(void)
{
    ...
    if (result != gBleSuccess_c) {
        QS_BEGIN_ID(BLE_HAL, 0)
            QS_STR("HAL_BLE_stopAdvertisement failed: ");
            QS_U32(0, result);
        QS_END()
        bleStatus = HAL_BLE_ERROR;
    } else {
 ...
 *
 * @note #1: For now, only the battery level is updated
 *
 * @note #2: Asynchronous State Machine is used to update the advertising data.
 *           This is due to NXP BLE stack design.
 *
...
HAL_BLE_status_t HAL_BLE_updateAdvertisingData(HAL_BLE_adServiceData_t advData)
{
    ...
    if (advUpdateState != ADV_UPDATE_IDLE) {
        ...
        QS_END()
        return HAL_BLE_SUCCESS; // Ignore new request and don't assert
    }
...
static void _HAL_BLE_DiscoverGattHandles(void)
{
    ...
    bleUuid_t aiosFullBatteryCharUuid;
    bleUuid_t aiosBank1CharUuid;
    ...
    g_gattHandles.aiosFullBatteryValueHandle = 0x0000;
    g_gattHandles.aiosBank1ValueHandle = 0x0000;
    ...
    // Find AIOS Full Battery characteristic value handle using NXP API with 128-bit UUID
    result = GattDb_FindCharValueHandleInService(
        g_gattHandles.aiosServiceHandle,
        gBleUuidType128_c,
        &aiosFullBatteryCharUuid,
        &g_gattHandles.aiosFullBatteryValueHandle
    );

    // Discover aiosBank1ValueHandle
    uint8_t aiosBank1Uuid128[] = {0xB1, 0x2B, 0x12, 0x99, 0x70, 0x41, 0x3E, 0x96, 0x26, 0x49, 0x44, 0x52, 0xDB, 0xD8, 0x0A, 0x17};
    FLib_MemCpy(aiosBank1CharUuid.uuid128, aiosBank1Uuid128, 16);
    // Find AIOS Bank 1 characteristic value handle using NXP API with 128-bit UUID
    result = GattDb_FindCharValueHandleInService(
        g_gattHandles.aiosServiceHandle,
        gBleUuidType128_c,
        &aiosBank1CharUuid,
        &g_gattHandles.aiosBank1ValueHandle
    );

    // Discover aiosBank2ValueHandle
    uint8_t aiosBank2Uuid128[] = {0xB2, 0x2B, 0x12, 0x99, 0x70, 0x41, 0x3E, 0x96, 0x26, 0x49, 0x44, 0x52, 0xDB, 0xD8, 0x0A, 0x17};
    FLib_MemCpy(aiosBank2CharUuid.uuid128, aiosBank2Uuid128, 16);
    // Find AIOS Bank 2 characteristic value handle using NXP API with 128-bit UUID
    result = GattDb_FindCharValueHandleInService(
        g_gattHandles.aiosServiceHandle,
        gBleUuidType128_c,
        &aiosBank2CharUuid,
        &g_gattHandles.aiosBank2ValueHandle
    );

    // Discover aiosBank3ValueHandle
    uint8_t aiosBank3Uuid128[] = {0xB3, 0x2B, 0x12, 0x99, 0x70, 0x41, 0x3E, 0x96, 0x26, 0x49, 0x44, 0x52, 0xDB, 0xD8, 0x0A, 0x17};
    FLib_MemCpy(aiosBank3CharUuid.uuid128, aiosBank3Uuid128, 16);
    // Find AIOS Bank 3 characteristic value handle using NXP API with 128-bit UUID
    result = GattDb_FindCharValueHandleInService(
        g_gattHandles.aiosServiceHandle,
        gBleUuidType128_c,
        &aiosBank3CharUuid,
        &g_gattHandles.aiosBank3ValueHandle
    );

    // Discover aiosBank4ValueHandle
    uint8_t aiosBank4Uuid128[] = {0xB4, 0x2B, 0x12, 0x99, 0x70, 0x41, 0x3E, 0x96, 0x26, 0x49, 0x44, 0x52, 0xDB, 0xD8, 0x0A, 0x17};
    FLib_MemCpy(aiosBank4CharUuid.uuid128, aiosBank4Uuid128, 16);
    // Find AIOS Bank 4 characteristic value handle using NXP API with 128-bit UUID
    result = GattDb_FindCharValueHandleInService(
        g_gattHandles.aiosServiceHandle,
        gBleUuidType128_c,
        &aiosBank4CharUuid,
...
static void _HAL_BLE_InitializeGattDatabase(void)
{
    ...
    result = GattServer_RegisterCallback(_HAL_BLE_GattServerCallback);
    if (result != gBleSuccess_c) {
        QS_BEGIN_ID(BLE_HAL, 0)
...
void HAL_BLE_updateAttribute(HAL_BLE_attribute_t *attr)
{
    ...
    switch (attr->attribute) {
        case HAL_BLE_ATTR_BATTERY_LEVEL:
        {
            ...
            bleResult_t writeResult = GattDb_WriteAttribute(g_gattHandles.batteryLevelValueHandle, attr->length, (uint8_t *)attr->p_value);
        ...
        {
            // Update AIOS Full Battery characteristic
            bleResult_t writeResult = GattDb_WriteAttribute(g_gattHandles.aiosFullBatteryValueHandle, attr->length, (uint8_t *)attr->p_value);
            HAL_ASSERT((writeResult == gBleSuccess_c), __FILE__, __LINE__);
            break;
        }
        ...
        {
            // Update AIOS Bank 1 characteristic
            bleResult_t writeResult = GattDb_WriteAttribute(g_gattHandles.aiosBank1ValueHandle, attr->length, (uint8_t *)attr->p_value);
            HAL_ASSERT((writeResult == gBleSuccess_c), __FILE__, __LINE__);
            break;
        }
        ...
        {
            // Update AIOS Bank 2 characteristic
            bleResult_t writeResult = GattDb_WriteAttribute(g_gattHandles.aiosBank2ValueHandle, attr->length, (uint8_t *)attr->p_value);
            HAL_ASSERT((writeResult == gBleSuccess_c), __FILE__, __LINE__);
            break;
        }
        ...
        {
            // Update AIOS Bank 3 characteristic
            bleResult_t writeResult = GattDb_WriteAttribute(g_gattHandles.aiosBank3ValueHandle, attr->length, (uint8_t *)attr->p_value);
            HAL_ASSERT((writeResult == gBleSuccess_c), __FILE__, __LINE__);
            break;
        }
        ...
        case HAL_BLE_ATTR_AIOS_BANK4_VALUE:
        {
            ...
            bleResult_t writeResult = GattDb_WriteAttribute(g_gattHandles.aiosBank4ValueHandle, attr->length, (uint8_t *)attr->p_value);
            HAL_ASSERT((writeResult == gBleSuccess_c), __FILE__, __LINE__);
            break;
        }
        ...
        default:
        {
            HAL_ASSERT(0, __FILE__, __LINE__);  // Unknown attribute
            break;
        }
    }
}
...
HAL_BLE_status_t HAL_BLE_send_notif(HAL_BLE_attribute_t *attr, uint16_t conn_id)
{
    ...
    switch (attr->attribute)
    {
        case HAL_BLE_ATTR_BATTERY_LEVEL:
        {
            handle = g_gattHandles.batteryLevelValueHandle;
            break;
        }
    ...
    bleResult_t result = GattServer_SendNotification(conn_id, handle);
...
static void _HAL_BLE_GenericCallback(gapGenericEvent_t* pGenericEvent)
{
    ...
    switch (pGenericEvent->eventType) {
        case gInitializationComplete_c:
        {
            ...
            /** Initialize GATT database when BLE host initialization is complete */
            _HAL_BLE_InitializeGattDatabase();

            /** Set advertising parameters after GATT DB is initialized */
            _HAL_BLE_SetupAdvertisingParameters();

            /** Call registered stack initialization callback */
        ...
        {
            QS_BEGIN_ID(BLE_HAL, 0)
                QS_STR("BLE Internal Error Occurred");
            QS_END()
            /** Call registered stack initialization callback */
        ...
        case gAdvertisingDataSetupComplete_c:
        {
            ...
            if (advUpdateState == ADV_UPDATE_SETTING_DATA) {
                ...
                _HAL_BLE_StartAdvertisingInternal();
            } else {
...
static void _HAL_BLE_AdvertisingCallback(gapAdvertisingEvent_t* pAdvertisingEvent)
{
    ...
    QS_BEGIN_ID(BLE_HAL, 0)
        QS_STR("Adv cbck triggered, cnt=");
        QS_U32(0, g_advertisingCallbackCount);
    QS_END()

    QS_BEGIN_ID(BLE_HAL, 0)
        QS_STR("BLE Adv Evt: type=");
    ...
    switch (pAdvertisingEvent->eventType) {
        case gAdvertisingStateChanged_c:
        {
            ...
            if (advUpdateState == ADV_UPDATE_STOPPING) {
...
#if BLE_EXTENDED_LOGS == 1
                QS_BEGIN_ID(BLE_HAL, 0)
                    QS_STR("Adv Stopped for Data Update");
                QS_END()
#endif // BLE_EXTENDED_LOGS

                isAdvertising = false;
                ble_advertising_state_changed_callback(HAL_BLE_ADV_STATE_OFF); // registered callback
                advUpdateState = ADV_UPDATE_SETTING_DATA;
                ...
                if (result != gBleSuccess_c) {
                    QS_BEGIN_ID(BLE_HAL, 0)
                        QS_STR("Failed to set adv data during update: ");
                        QS_U32(0, result);
                    QS_END()
                ...
                QS_BEGIN_ID(BLE_HAL, 0)
                    QS_STR("Adv Started with Updated Data");
                QS_END()
#endif // BLE_EXTENDED_LOGS
                ...
                QS_BEGIN_ID(BLE_HAL, 0)
                    QS_STR("Adv data update success - battery level: ");
...
static void _HAL_BLE_GattServerCallback(deviceId_t deviceId, gattServerEvent_t* pServerEvent)
{
    ...
    switch (pServerEvent->eventType) {
        case gEvtMtuChanged_c:
        {
            ...
            // Call registered MTU changed callback
            if (ble_mtu_changed_callback != NULL) {
                ble_mtu_changed_callback(newMtu);
            }
            break;
        }
        ...
        case gEvtCharacteristicCccdWritten_c:
        {
            ...
            if (handle == g_gattHandles.batteryLevelCccdHandle) {
                ...
                bool isBatteryServiceSubscribed = (cccdValue == gCccdNotification_c);
                ...
                // Notify BAS
                BAS_handleCccdWritten(cccdValue);
            } 
